...
| True background job queue for uploads | Skip — upload returns immediately (fire-and-forget async function on the same process), not a real worker/queue | Solves the request-timeout and spinner-UX problem without new infra. Does **not** solve event-loop contention (huge PDF parsing is still CPU work on the API's single Node thread) or process-restart durability: if the container restarts mid-upload (redeploy, crash, `docker compose up --build`), the in-memory work is lost and the `TutorPack` row would be stuck `PROCESSING` forever with no original file bytes to resume from. Mitigated with a startup reconciliation (`index.ts`) that marks any `PROCESSING` pack `FAILED` on boot, so a restart produces a visible, re-uploadable failure instead of a silent zombie — found via real testing on the Azure VM where several rebuilds during iteration left orphaned stuck packs. Acceptable for admin-only, low-concurrency uploads; move to a `worker_thread` or real job queue (with resumable state) if this ever becomes a real problem. |
