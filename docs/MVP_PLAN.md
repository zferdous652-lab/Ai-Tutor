...
| True background job queue for uploads | Skip — upload returns immediately (fire-and-forget async function on the same process), not a real worker/queue | Solves the request-timeout and spinner-UX problem without new infra. Does **not** solve event-loop contention (huge PDF parsing is still CPU work on the API's single Node thread) or process-restart durability: if the container restarts mid-upload, the in-memory work is lost and the `TutorPack` row would be stuck `PROCESSING` forever with no original file bytes to resume from — mitigated with a startup reconciliation (`index.ts`) that marks any `PROCESSING` pack `FAILED` on boot. Separately, real testing on the Azure VM also hit a genuine in-flight hang (not a restart) — a 12-page upload sat at `PROCESSING` with the API container at ~0% CPU, meaning `extractPdfMarkdown` (`@opendocsg/pdf2md`, itself pdf.js-based) got stuck awaiting something that never resolved, for reasons not yet root-caused. Mitigated with a hard timeout (`lib/timeout.ts`, 5 min for extraction / 15 min for visual analysis) around each stage, so a hang now becomes a visible `FAILED` within a bounded time instead of silently forever — but the timeout doesn't fix *why* pdf2md hangs on some real-world PDFs, it only bounds the damage. Acceptable for admin-only, low-concurrency uploads; move to a `worker_thread` or real job queue (with resumable state, and real cancellation) if this becomes a recurring problem. |
| Precise per-chapter attribution of visual notes | Skip — captioned diagrams/maps/photos are attached to the whole Tutor Pack, not mapped to the specific chapter they appear in, and are appended as shared context to every chapter's generation prompt | Chapters are detected from Markdown headings (services/pdf.ts), which don't carry the original PDF page numbers a vision pass renders against; precisely reconciling the two needs deeper pdf.js integration. The whole-pack appendix is simpler and still solves the core problem (visual content becoming visible to the AI at all), at the cost of some irrelevant context on chapters that don't contain the referenced figure. |
