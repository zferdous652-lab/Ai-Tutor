...
| True background job queue for uploads | Skip — upload returns immediately (fire-and-forget async function on the same process), not a real worker/queue | Solves the request-timeout and spinner-UX problem without new infra. Does **not** solve event-loop contention: parsing a huge PDF is still CPU work on the API's single Node thread, so other requests can see latency during that window. Acceptable for admin-only, low-concurrency uploads; move to a `worker_thread` or real job queue if concurrent large uploads ever cause noticeable impact. |
